...
document.addEventListener('DOMContentLoaded',async ()=>{
  ...
  try {
    ...
    const mostrarR= await axios.get('/api/secadoras/consultarReservas')
    console.log(mostrarR);
       ...
       tr.innerHTML=`
                 ...
                 <td class="py-3  text-center ">${item.secadora}</td>
...
const nroSecadora=document.querySelector('#nroSecadora')
...
nroSecadora.addEventListener('input', filtrarReservas);

function filtrarReservas() {
  // Get the payments data from the API
  axios.get('/api/secadoras/consultarReservas')
   .then(response => {
      ...
      const nroSecadoraValue = nroSecadora.value.trim(); // Debes asegurarte de que nroLavadora sea un elemento HTML válido
      ...
      const filteredReservas = reservas.filter(item => {
        ...
        if (nroSecadoraValue &&!item.secadora.toString().includes(nroSecadoraValue)) {
          match = false;
        }
